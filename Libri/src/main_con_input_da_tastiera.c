...
int main(int argc, char *argv[])
{
	...
	for (int i=0; i<n; i++){
		...
		printf("Inserisci l'editore del %d libro\n", i+1);
		fgets(buffer,100,stdin);
		buffer[strlen(buffer) - 1] = '\0';
		strcpy(ed,buffer);

		printf("Inserisci il prezzo del %d libro\n", i+1);
		fgets(buffer,100,stdin);
		pr = atof(buffer);

		printf("Inserisci l'anno del %d libro\n", i+1);
		fgets(buffer,100,stdin);
		an = atoi(buffer);
	...
	printf("Inserisci l'editore da cercare\n");
	fgets(ed,50,stdin);
	ed[strlen(ed) - 1] = '\0';

	int trovati=0;
	libro *libriEd = ricercaeditore(ed,biblioteca,n,&trovati);
	if(trovati != 0)
...
libro* ricercaeditore(char* ed, libro* biblioteca, int n, int* t){
	...
	if(trovati == 0)
		return 0;
	*t = trovati;
	libriEditore = malloc(sizeof(libro) * trovati);
	trovati = 0;
